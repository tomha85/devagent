...
def test_real_asyncua_production_subscription_and_coherent_snapshot() -> None:
    async def scenario() -> None:
        ...
        async with OpcUaSimulator(endpoint=endpoint, update_interval_seconds=0.05) as simulator:
            ...
            try:
                ...
                await manager.replace_monitored_node_ids("plc1", nodes)
                ...
                events = manager.drain_realtime_events(
                    "plc1", node_ids=nodes, max_events=5000
                )
                assert events
...
def test_real_asyncua_two_plc_production_manager_reads_both_sessions() -> None:
    async def scenario() -> None:
        ...
        async with OpcUaSimulator(
            endpoint=endpoint1, scenario="normal", update_interval_seconds=0.05
        ) as sim1, OpcUaSimulator(
            endpoint=endpoint2, scenario="normal", update_interval_seconds=0.05
        ) as sim2:
            ...
            try:
                ...
                await manager.replace_monitored_node_ids("plc1", nodes1)
                await manager.replace_monitored_node_ids("plc2", nodes2)
                await asyncio.sleep(0.30)
                results = await manager.read_many({"plc1": nodes1, "plc2": nodes2})
...
def test_monitor_capacity_contract_handles_256_nodes_deterministically() -> None:
    async def scenario() -> None:
        ...
        await manager.replace_monitored_node_ids("plc1", requested)
        ...
        await manager.replace_monitored_node_ids("plc1", replacement)
