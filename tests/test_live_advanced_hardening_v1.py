...
def test_rll_comparator_truth_does_not_claim_output_conflict():
    ...
    diagnosis = diagnose_numeric_comparison(
        comparison,
        {
            ...
            # The rung may contain another false contact, so a false coil is not a conflict.
...
def test_historical_advanced_question_is_not_forced_through_boolean_target_limitation():
    source = inspect.getsource(recursive_assistant.RecursiveLiveCommissioningAssistant._historical_reply)
    assert "resolve_advanced_target(self.advanced_coverage, text).found" in source
