...
class RealtimeMultiPlcConnectionManager(MultiPlcConnectionManager):
    ...
    * concurrent subscription/read cache writes are ordered per node;
    * cache/event state is invalidated whenever connection continuity is lost;
    * subscription events are retained in a bounded queue for the historical layer.
    ...
    @staticmethod
    def _cache_observed_at(value: RuntimeValue) -> datetime:
        candidates = tuple(
            stamp
            for stamp in (value.source_timestamp, value.server_timestamp)
            ...
            try:
                ...
                async with await client.create_subscription(self.publishing_interval_ms) as subscription:
                    ...
                    while generation == state.generation:
                        ...
                        self._update_cache(state, value, source="SUBSCRIPTION")
    ...
    def _cached_snapshot(
        ...
        for node_id in node_ids:
            ...
            if (
                ...
                return None
            # RuntimeValue.stale captures trust/staleness when the value entered
            # the cache. OPC UA SourceTimestamp may represent the last source
            # change and can therefore be old for a stable but freshly received
            # value. Cache residency is intentionally based on local receipt time.
    ...
    async def _batch_read_isolated(
        ...
        async with entry.lock:
            ...
            if outer_client is None or not bool(getattr(outer_client, "connected", False)):
                ...
                self._set_state(entry, observed.state, error=reason)
