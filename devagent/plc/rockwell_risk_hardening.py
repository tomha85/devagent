    ...
    for component, representatives in _writer_components(project):
        ...
        result.append(
            RiskFinding(
                ...
                Severity.MEDIUM,
                f"Overlapping Rockwell storage is written by {len(evidence_ids)} distinct executable sources after scope/case/member normalization and AliasFor normalization.",
    ...
    for identity, evidence_ids in sorted(latched.items(), key=lambda item: item[0]):
        ...
        result.append(
            RiskFinding(
                ...
                Severity.MEDIUM,
                "An OTL writer was found without a corresponding OTU after Rockwell scope/case/member normalization and AliasFor normalization.",
