...
def render_semantic_coverage_section(project) -> str:
    ...
    lines = [
        ...
        f"- Stateful timer/counter FAT models: **{stateful['modeled_occurrences']}**",
        f"- Stateful engineer FAT required: **{'yes' if stateful['requires_qualified_runtime_evidence'] else 'no'}**",
        f"- Motion FAT contracts: **{motion.get('modeled_occurrences', 0)}**",
        f"- Motion engineer FAT required: **{'yes' if motion.get('requires_qualified_runtime_evidence') else 'no'}**",
        ...
        f"- State-machine engineer FAT required: **{'yes' if state_machine.get('runtime_evidence_required') else 'no'}**",
