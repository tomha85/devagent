...
def _render(manifest: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        "SEMANTIC COVERAGE",
        f"Instruction occurrences: {summary['total_occurrences']}",
        f"Deterministic instruction coverage: {_pct(summary['deterministic_pct'])}",
        f"Structural-or-better coverage: {_pct(summary['structural_or_better_pct'])}",
        f"Structural-only occurrences: {summary['structural_only_occurrences']}",
        f"Partial occurrences: {summary['partial_occurrences']}",
        f"Unmodeled occurrences: {summary['unmodeled_occurrences']} ({_pct(summary['unmodeled_pct'])})",
        "",
        "LANGUAGES / EXECUTION SURFACES",
        f"RLL rungs: {rll['rungs']}",
        ...
        f"ST FULL dataflow statements: {st['full_dataflow_statements']}",
        f"ST partial statements: {st['partial_statements']}",
        f"ST opaque statements: {st['opaque_statements']}",
